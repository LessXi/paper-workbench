...
export const name = 'paper-workbench'
export const inject = ['tools']

/** 在当前工作目录（论文工作台）定位解析环境 */
function locate(workdir: string) {
  const root = resolve(workdir)
  const toolsDir = join(root, '.zcode', 'tools')
  const py = existsSync(join(toolsDir, '.venv', 'Scripts', 'python.exe'))
    ? join(toolsDir, '.venv', 'Scripts', 'python.exe')
    : join(toolsDir, '.venv', 'bin', 'python')
  return {
    root,
    isWorkspace: existsSync(join(root, 'registry.md')) && existsSync(join(root, 'library')),
    venvReady: existsSync(py),
    mineruReady: existsSync(join(toolsDir, '.venv', 'Lib', 'site-packages', 'mineru')),
    py,
    script: join(toolsDir, 'parse_paper.py'),
  }
}
...
function run(cmd: string, args: string[], timeoutMs: number): Promise<string> {
  return new Promise((resolvePromise, reject) => {
    execFile(cmd, args, { timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024, windowsHide: true }, (err, stdout, stderr) => {
      if (err) reject(new Error(`paper: ${err.message}\n${String(stderr).slice(-2000)}`))
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    name: 'paper',
    description:
      '论文工作台工具。action=status 检查当前目录工作台状态（数据层/解析环境/档位）；' +
    ...
    execute: async (args: { action: string; pdf?: string; engine?: string; workdir?: string }) => {
      ...
      if (args.action === 'parse') {
        if (!loc.venvReady) throw new Error('paper: 解析环境不存在，先运行 /paper-init')
        if (!args.pdf) throw new Error('paper: parse 需要 pdf 参数')
        const engineArgs = args.engine === 'fast' ? ['--engine', 'fast'] : []
        return run(loc.py, [loc.script, ...engineArgs, resolve(args.pdf)], 900_000)
      }
